...
def copy_conv(sess, tftensor, layer):
    ...
    W = sess.graph.get_tensor_by_name('{}/conv2d_params:0'.format(tftensor))\
        .eval()
...
def copy_bn(sess, tftensor, layer):
    ...
    eps = sess.graph.get_operation_by_name(tftensor)\
        .get_attr('variance_epsilon')
...
def set_tf_params(model, write_graph=False):
    ...
    tf.io.gfile = tf.compat.v1.gfile
